...
def train(G: torch.nn.Module,
    ...
    for batch_index, minibatch in tqdm(enumerate(trainloader), desc='Running training'):
        ...
        fake_images = G(torch.randn(batch_size, latent_dim, 1, 1).cuda())

        # The Generator's success criteria - generate image that looks real
        if g_mode == 'original':
            generator_criteria = adversarial_loss(1 - D(fake_images).reshape(-1, 1), zeros_val)
        else:
            generator_criteria = adversarial_loss(D(fake_images).reshape(-1, 1), ones_val)
        ...
        real_loss = adversarial_loss(D(real_imgs).reshape(-1, 1), ones_val)
        fake_loss = adversarial_loss(D(fake_images.detach()).reshape(-1, 1), zeros_val)
...
def test(G: torch.nn.Module,
    ...
    with torch.no_grad():
        for batch_index, minibatch in enumerate(testloader):
            ...
            # The Discriminator's success criteria - correctly indentify real and fake images
            real_loss = adversarial_loss(D(real_images).reshape(-1, 1), ones_val)
            fake_loss = adversarial_loss(D(fake_images.detach()).reshape(-1, 1), zeros_val)
            discriminator_criteria = (real_loss + fake_loss) / 2
            epoch_loss_d += discriminator_criteria.item()
...
def main(args):
    ...
    G = CGAN.Generator(latent_dim=args.latent_dim, batch_size=args.batch_size, hidden_dim=args.g_hid_dim, output_dim=1).cuda()
    G.apply(CGAN.weights_init)

    D = CGAN.Discriminator(input_dim=1, hidden_dim=args.d_hid_dim).cuda()
    D.apply(CGAN.weights_init)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % SAMPLE_INTERVAL == 0:
            samples = sample(G, args.sample_size, args.latent_dim)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr',
                        ...
                        type=float,
                        default=2e-4)
    parser.add_argument('--g_hid_dim',
                        help='generator hidden dim',
                        type=int,
                        default=32)
    parser.add_argument('--d_hid_dim',
                        help='discriminator hidden dim',
                        type=int,
                        default=32)
